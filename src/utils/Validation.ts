...
export const editUserSchema: Yup.SchemaOf<IEditUserSchema> = Yup.object().shape(
    {
        name: Yup.string().max(64, 'نام نباید بیشتر از 64 کاراکتر باشد'),
        family: Yup.string().max(64, 'نام نباید بیشتر از 64 کاراکتر باشد'),
        skills: Yup.string(),
        about: Yup.string(),
    }
)
...
export const createPostSchema: Yup.SchemaOf<bookMarkModuleSchema> =
    Yup.object().shape({
        ...
        tag: Yup.string().max(
            32,
            'حداکثر کارکتر های ورودی نباید بیشتر 32 باشد'
        ),
    })
